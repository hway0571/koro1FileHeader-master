...
class FunctionTplStr {
  ...
  paramStr (key) {
    ...
    // 用户自定义的字段（如"函数名称"、"函数功能"、"传入参数"、"返回参数"等）始终显示
    // NoMatchParams配置只对param/return字段生效，不影响用户自定义字段
    return new LanguageDifferent(obj).res
  }
  ...
  handleParamReturn (obj, key) {
    ...
    // 检查值是否为非数组（即没有匹配到函数参数）
    const isNotArray = !Array.isArray(this.data[key])
    
    // 只处理插件默认的 param/return 字段，不处理用户自定义字段
    // 用户自定义字段（如"传入参数"、"返回参数"）应该通过 fnMiddle_key 方法处理
  ...
  paramsHandle (obj, key) {
    ...
    // 当值不是数组且为空字符串或空格时，不显示该行
    if (paramArr === '' || paramArr === ' ' || paramArr === undefined) {
      const NoMatchParams = this.config.configObj.NoMatchParams
      if (NoMatchParams === 'no show param' || NoMatchParams === 'no show param and return') {
        return ''
      }
    }
    return new LanguageDifferent(obj).res
