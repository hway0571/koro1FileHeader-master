...
class FunctionTplStr {
  ...
  paramStr (key) {
    ...
    // 只处理插件默认的 param/return 字段（完全匹配，不是 startsWith）
    // 用户自定义字段（如"传入参数"、"返回参数"、"param_desc"等）应该通过 fnMiddle_key 处理
    if (key === 'param' || key === 'return') {
      return this.handleParamReturn(obj, key)
    }
    
    // 用户自定义的字段始终通过 fnMiddle_key 处理
    return new LanguageDifferent(obj).res
  }
  ...
  paramsHandle (obj, key) {
    ...
    // 对于非数组值，直接渲染（用户自定义字段应该通过 fnMiddle_key 方法处理）
    return new LanguageDifferent(obj).res
  }
