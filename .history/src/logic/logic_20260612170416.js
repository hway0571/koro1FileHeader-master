...
const cursorOptionHandleFn = (config) => {
  ...
  // 调试：检查用户配置是否正确读取
  console.log('=== cursorMode 配置调试 ===')
  console.log('userObj:', JSON.stringify(userObj))
  console.log('userObj 键:', Object.keys(userObj))
  console.log('userObj 键数量:', Object.keys(userObj).length)
  ...
  if (Object.keys(userObj).length === 0) {
    ...
    // 如果用户设置了模板，那将默认根据用户设置模板
    data = Object.assign({}, userObj) // 复制对象，否则对象不能更改值
    console.log('使用用户配置:', JSON.stringify(data))
  }
  ...
  // 调试：检查 changePrototypeNameFn 之前的数据
  console.log('changePrototypeNameFn 之前:', JSON.stringify(data))
  
  data = logicUtil.changePrototypeNameFn(data, config)
  
  // 调试：检查 changePrototypeNameFn 之后的数据
  console.log('changePrototypeNameFn 之后:', JSON.stringify(data))
  
  data = changeTplValue(data, config) // 修改模板设置的值
  
  // 调试：检查 changeTplValue 之后的数据
  console.log('changeTplValue 之后:', JSON.stringify(data))
  
  data = logicUtil.sameLengthFn(data, 'function') // 将字段弄得一样长
  
  // 调试：检查最终数据
  console.log('最终数据:', JSON.stringify(data))
  console.log('最终数据键:', Object.keys(data))
