...
class GetParams {
  init (lineProperty) {
    ...
    this.functionName = '' // 函数名
    this.returnType = '' // 返回类型
    this.res = ''
    this.matchProcess()
  }
  ...
  matchProcess () {
    ...
    for (const item of keyArr.values()) {
      ...
      if (match) {
        ...
        // 提取函数名
        if (match[2]) {
          this.functionName = match[2].trim()
        }
        // 提取返回类型
        if (match[1]) {
          this.returnType = match[1].trim()
        }
        break
      }
    }
  ...
  // 匹配方法声明的参数
  matchFunction () {
    // 函数名 可能有空格 匹配一个括号 匹配一个括号 可能有空格 类型可能的值: [\w[].] 匹配2个字符以上 必须的空格 匹配一个参数名\w 到这里已经判定它是函数声明后面匹配括号内的其他一切
  ...
  parsing (params) {
    ...
    // 如果参数只有 void，则表示无参函数，不添加参数
    // 使用正则匹配，确保能处理各种空白字符（包括全角空格）
    if (/^\s*void\s*$/.test(params)) {
      this.res = paramsArr
      this.match = false
      return
    }
