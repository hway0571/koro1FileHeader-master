  ...
  matchProcess () {
    ...
    for (const item of keyArr.values()) {
      ...
      if (match) {
        ...
        params = match[index]
        // 提取函数名
        if (item === 'matchFunction' && match[1]) {
          this.functionName = match[1].trim()
        } else if (item === 'arrowFunction' && match[1]) {
          this.functionName = match[1].trim()
        } else if ((item === 'matchClassFunction') && match[2]) {
          this.functionName = match[2].trim()
        } else if (item === 'matchObjFunction' && match[1]) {
          this.functionName = match[1].trim()
        }
        break
      }
